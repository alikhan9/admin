...
import { UserListHead, UserListToolbar, UserMoreMenu, CreateGroupForm, EditUserForm } from '../sections/@dashboard/groups';
// client
...
export default function Groups() {
  ...
  const requestAllUsers = () => {
    getAllUsers()
    .then(response => {
      response.data.includes('html') ? navigate('/login', { replace: true }) : setGroups(response.data);
    })
  ...
  return (
    <Page title="User">
      <Container maxWidth="xxl" style={{ height: 1, marginTop:  -50}} >
        ...
        <CreateGroupForm requestAllUsers={requestAllUsers} navigate={navigate} open={open} handleClose={handleClose}/>
        <EditUserForm open={openEdit} handleClose={handleCloseEdit} editRow={editRow} setGroups={setGroups} />
        <Card sx={{ width: 1 }}>
          <UserListToolbar setSelected={setSelected} selected={selected} navigate={navigate} setGroups={setGroups} filterName={filterName} onFilterName={handleFilterByName} />
          <Scrollbar>
              ...
              <Table>
                ...
                <TableBody>
                  {filteredUsers?.slice(page * rowsPerPage, page * rowsPerPage + rowsPerPage).map((row) => {
                    const { id, username, first_name, last_name, email, dateofbirth } = row;
                    ...
                    return (
                      ...
                      >
                        ...
                        <TableCell align="left">
                          <UserMoreMenu setGroups={setGroups} navigate={navigate} handleOpen={handleOpenEdit} row={row} setEditRow={setEditRow} />
                        </TableCell>
                      </TableRow>
                    );
                  })}
